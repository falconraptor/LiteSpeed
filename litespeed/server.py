...
class WebServer(ThreadingTCPServer):
    ...
    def setup_env(cls, port: int):
        cls.base_environ = cls.base_environ or Request({'SERVER_NAME': socket.gethostname(), 'GATEWAY_INTERFACE': 'CGI/1.1', 'SERVER_PORT': str(port), 'REMOTE_HOST': '', 'CONTENT_LENGTH': '', 'SCRIPT_NAME': ''})
    ...
    def client_left(self, handler: 'RequestHandler'):
        try:
            self.handle(client := self.handlers[id(handler)], 'left')
        ...
        for client in self.clients.values():
            if client['handler'].connection._closed:
                del self.clients[client['id']]
                del self.handlers[client['handler_id']]
        for client in self.handlers.values():
            if client['handler'].connection._closed:
                del self.clients[client['id']]
                del self.handlers[client['handler_id']]
...
class App:
    ...
    def __call__(self, env: dict, start_response: Callable):
        path = env['PATH_INFO']
        f_list = self._handle_route_cache(env := Request(env))
        headers = {}
        ...
        try:
            ...
            for f in f_list:
                if path[-1:] != '/' and 'result' not in locals() and not f[0].no_end_slash:  # auto rediects to url that ends in / if no_end_slash is False
                    ...
                    if not (r := self._handle_cors(f, headers, env)) or ('*' not in f[0].methods and env['REQUEST_METHOD'].lower() not in f[0].methods):  # checks for allowed methods
                        result = self.error_routes.get(405, (lambda e, *a, **k: (b'', 405, {'Content-Type': 'text/public'})))(env, *f[1], **f[2])
                    else:
                        result = f[0](env, *f[1], **f[2])
                        called = True
                if (l_result := len(result or [])) <= 1 or l_result > 3 or isinstance(result, dict) or (l_result >= 2 and result[1] != 405):
                    break
        ...
        if called and (status := int(r[1].split()[0])) in self.error_routes and status not in f[0].disable_default_errors:
    ...
    def _handle_cors(self, f: Callable, headers: dict, env: dict) -> bool:
        def _check_cors() -> bool:
            if cors := (f[0].cors or self._cors_origins_allow):  # checks for cors allowed domains using route override of global
                if '*' in cors:
        ...
        if methods := (f[0].cors_methods or self._cors_methods_allow):  # checks for cors allowed methods using route override of global
            if '*' in methods:
    ...
    def _handle_result(self, result, headers: dict, cookie: set, env: Request) -> Tuple[List[bytes], str, List[tuple]]:
        ...
        def _handle_compression():
            ...
            if 'gzip' in env.get('ACCEPT_ENCODING', '').lower() and body_len > 200 and 'image' not in headers.get('Content-Type', '').lower():
                ...
                if (compressed_len := len(compressed_body)) < body_len:
    ...
    def _handle_route_cache(self, env: Request) -> List[Tuple[Callable, Union[Generator, List[str]], Dict[str, str]]]:
        path = env.PATH_INFO
        if not (result := self.__route_cache.get(path, [])):  # finds url from urls and adds to ROUTE_CACHE to prevent future lookups
            for url in self._urls:
                tmp = path + ('/' if not url.no_end_slash and path[-1] != '/' and url.re.pattern[-1] == '/' else '')
                if m := url.re.fullmatch(tmp[1:]) if tmp and tmp[0] == '/' and url.re.pattern[0] != '/' else url.re.fullmatch(tmp):
                    group_dict = m.groupdict()
                    group_values = group_dict.values()
                    groups = [g for g in m.groups() if g not in group_values]
                    try:
                        result = [(url, groups, group_dict)]
                        self.__route_cache[path].append(result[0])
                    except KeyError:
                        result = self.__route_cache[path] = [(url, groups, group_dict)]
    ...
    def route(cls, url: Optional[str] = None, methods: Union[Iterable, str] = '*', function: Callable = None, cors: Optional[Union[Iterable, str]] = None, cors_methods: Optional[Union[Iterable, str]] = None, no_end_slash: bool = False, disable_default_errors: Optional[Iterable[int]] = None):
        ...
        def decorated(func) -> partial:
            nonlocal url
            url = url or ((func.__module__.replace('.', '/') + '/').replace('__main__/', '') + (func.__name__ + '/').replace('index/', ''))
...
class RequestHandler(BaseHTTPRequestHandler):
    ...
    def _get_boundary_enclosed(boundary: List[str], env: Request):
        ...
        while not line or isinstance(line, bytes) or RE_DASHES.sub('', line, 1) != boundary:
            ...
            try:
                if decoded := line.decode().replace('\r', '').replace('\n', ''):
                    if RE_DASHES.sub('', decoded, 1) in {boundary[:-2], boundary}:
                        ...
                        if file:
                            file.seek(0)
                    if not content:
                        name = name or (RE_NAME.findall(decoded) or [''])[0]
                        if not filename:
                            filename = RE_FILENAME.findall(decoded)
                            if filename:
                                filename = filename[0]
                                file = BytesIO()
                        if decoded[:12] == 'Content-Type':
            ...
            if skip > 0:
                ...
                i += 1
                continue
            if content and (not decoded or (decoded[:12] != 'Content-Type')):
    ...
    def get_environ(self) -> Request:
        ...
        :returns:Request"""
        env = Request({'SERVER_PROTOCOL': self.request_version, 'SERVER_SOFTWARE': self.server_version, 'REQUEST_METHOD': self.command.upper(), 'BODY': b'', 'GET': {}, 'POST': {}, 'PATCH': {}, 'PUT': {}, 'OPTIONS': {}, 'DELETE': {}, 'FILES': {}, 'COOKIE': SimpleCookie(self.headers.get('COOKIE')), 'HEADERS': {k.upper().strip(): v for k, v in self.headers.items()}, 'REMOTE_ADDR': self.client_address[0], 'CONTENT_TYPE': self.headers.get_content_type(), 'HOST': self.headers['HOST']})
        path, env['QUERY_STRING'] = self.path.split('?', 1) if '?' in self.path else (self.path, '')
        env['PATH_INFO'] = unquote_plus(path, 'iso-8859-1')
        if (host := env['HEADERS'].get('X-REAL-IP') or env['HEADERS'].get('X-FORWARDED-FOR', '').split(',')[-1].strip() or self.address_string()) != self.client_address[0]:
            env['REMOTE_HOST'] = host
            self.client_address = (host, self.client_address[1])
        env['CONTENT_LENGTH'] = int(self.headers.get('content-length', '0'))
        if (l_env_body := len(env['BODY'])) != env['CONTENT_LENGTH']:
            env['BODY'] += self.rfile.read(env['CONTENT_LENGTH'] - l_env_body)
        if boundary := re.findall(r'boundary=-*(\w+)', self.headers.get('content-type', '')):  # boundary is used to catch multipart form data (includes file uploads)
            self._get_boundary_enclosed(boundary, env)
        elif (content_type := env['CONTENT_TYPE'].lower()) == 'application/json' and env['BODY']:
            ...
            for q in re.sub(r'-{15,}\d+', '+@~!@+', env['BODY'].decode().replace('\n', '')).split('+@~!@+'):
                if '=' in q:
                    k, v = [unquote_plus(a) if a else a for a in q.split(';')[1].strip().split('=', 1)[1].replace('"', '').split('\r\r')]
                    v = v.replace('\r', '')
    ...
    def read_next_message(self):
        ...
        try:
            ...
            b1, b2 = 0, 0
        if (opcode := b1 & 0x0f) == 0x8 or not b2 & 0x80:  # disconnect
            self.keep_alive = False
            return
    ...
    def send_message(self, message: Union[bytes, str], opcode: int = 0x1) -> bool:
        ...
        if (payload_length := len(payload)) <= 125:  # Normal payload
